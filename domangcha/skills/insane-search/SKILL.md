...
<!-- DOMANGCHA vendored copy of insane-search (upstream: fivetaku/insane-search, MIT).
     The upstream plugin's Step-0 GitHub-star prompt and ${CLAUDE_PLUGIN_ROOT} setup
     hook were removed for vendored use; engine behavior is unchanged. -->
**Step 0 — engine root:** this skill ships its own `engine/`. Every `python3 -m engine ...`
command below runs **from the skill directory** — `~/.claude/skills/insane-search/`
(installed by DOMANGCHA `install.sh`). Always `cd` there, or pass the absolute path,
before invoking the engine.

**Dependencies (idempotent, non-blocking — run once if the engine fails to import):**
`bash ~/.claude/skills/insane-search/setup/setup.sh` — env check only; it never prompts
and is safe to skip when `python3 -c "import engine"` already succeeds from the skill dir.
